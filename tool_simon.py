...
'''
    ...
    custom_data.data[:,3]  = custom_data[:, 5]
    custom_data.data[:, 5] = tmp
    #custom_data.data[:, 5] = -custom_data.data[:, 5]

    # ref_vel from NED to ENU
    custom_data.data[:, 6:9] = np.matmul(ned2enu_rotation_matrix, sim.dmgr.ref_vel.data)
    ...
    custom_data.data[:, 9:12] = np.matmul(ned2enu_rotation_matrix, sim.dmgr.ref_accel.data)
    ...
    custom_data.data[:, 12:15] = np.matmul(ned2enu_rotation_matrix, sim.dmgr.ref_gyro.data)
...
if __name__ == '__main__':
    ...
    v = np.array([[45], [10], [45]])
    v2 = np.matmul(ned2enu_rotation_matrix, v)
    print(v2)
    '''
